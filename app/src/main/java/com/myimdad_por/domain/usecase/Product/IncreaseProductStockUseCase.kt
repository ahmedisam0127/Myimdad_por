...
class IncreaseProductStockUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val safeAmount = amount
            ?: return Result.failure(IllegalArgumentException("قيمة الزيادة غير موجودة"))

        if (safeAmount <= BigDecimal.ZERO) {
            return Result.failure(IllegalArgumentException("قيمة الزيادة يجب أن تكون أكبر من صفر"))
        }
    ...
    companion object {
        ...
        private const val DEFAULT_REASON = "زيادة مخزون"
    }
}