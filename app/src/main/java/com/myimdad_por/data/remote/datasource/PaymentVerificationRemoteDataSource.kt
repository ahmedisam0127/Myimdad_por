...
class PaymentVerificationRemoteDataSource @Inject constructor(
    ...
    suspend fun verifyPayment(request: PaymentVerificationDto): NetworkResult<PaymentVerificationDto> {
        return call { apiService.verifyPayment(request) }
    }

    suspend fun getPaymentVerification(transactionId: String): NetworkResult<PaymentVerificationDto> {
        return call { apiService.getPaymentVerification(transactionId) }
    }
    ...
    suspend fun execute(
        request: suspend PaymentApiService.() -> Response<PaymentVerificationDto>
