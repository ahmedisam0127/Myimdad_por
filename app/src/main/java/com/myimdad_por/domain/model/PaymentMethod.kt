...
sealed class PaymentMethod(
    ...
    /**
     * اسم العرض باللغة العربية بناءً على النوع
     * هذا يحل مشكلة الـ displayName التي طلبها الـ UI
     */
    val displayName: String
        get() = when (this) {
            is CASH -> "نقداً"
            is BANK_TRANSFER -> "تحويل بنكي"
            is WALLET -> "محفظة إلكترونية"
            is POS -> "نقطة بيع (POS)"
            is CHEQUE -> "شيك"
            is Custom -> providerName ?: name
        }
    ...
    companion object {
        /**
         * دالة بديلة لـ .values() لأن الـ Sealed Class لا تدعمها تلقائياً
         * تستخدم في شاشة المبيعات لعرض الخيارات
         */
        fun values(): List<PaymentMethod> {
            return listOf(CASH, BANK_TRANSFER, WALLET, POS, CHEQUE)
        }
    }
}
